...
extension UIImage {
    ...
    func imageSizeValidation() -> Bool {

        let imageData = self.jpegData(compressionQuality: 0.7)
        let imageSizeMB = Double(imageData?.count ?? 0) / 1024 / 1024
...
extension UIImage {
    ...
    /// - Returns: A new UIImage with the blur effect applied, or `nil` if the operation fails.
    func blur(_ radius: Double) -> UIImage? {
        ...
        // Crop the blurred image to match the original image's extent
        let newImageSize = CGRect(
            x: outputImage.extent.origin.x + (outputImage.extent.size.width - self.size.width) / 2,
            y: outputImage.extent.origin.y + (outputImage.extent.size.height - self.size.height) / 2,
            width: self.size.width,
            height: self.size.height
        )

        let context = CIContext()
        guard let cgimg = context.createCGImage(outputImage, from: newImageSize) else { return nil }
...
extension UIImage {
    ...
    public class func gif(name: String, completion: (() -> Void)?) -> UIImage? {
    ...
    internal class func animatedImageWithSource(_ source: CGImageSource) -> UIImage? {
        ...
        if let gifCompletionHandler = objc_getAssociatedObject(self, &AssociatedObjectKey.gifCompletionHandler) as? (() -> Void) {
            DispatchQueue.CGCDMainThread.asyncAfter(deadline: (.now() + Double(duration) / 1000.0), execute: gifCompletionHandler)
        }
