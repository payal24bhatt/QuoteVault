...
extension UITextView: UITextViewDelegate {
    ...
    @IBInspectable var placeholder: String? {
        ...
        get {
            ...
            if let placeholderLabel = self.viewWithTag(AssignedTags.textfieldPlaceholder) as? UILabel {
    ...
    private func addPlaceholderLabel(placeholder: String?, placeholderColor: UIColor?, placeholderFont: UIFont?) {
        ...
        let placeholderLabel = UILabel(frame: CGRect(x: 0, y: 0, width: self.frame.width, height: CGFloat.greatestFiniteMagnitude))
        placeholderLabel.textAlignment = self.textAlignment
        placeholderLabel.numberOfLines = 0
        placeholderLabel.tag = AssignedTags.textfieldPlaceholder
    ...
    open override func willMove(toSuperview newSuperview: UIView?) {
        ...
        if newSuperview == nil {
            if (self.viewWithTag(AssignedTags.textfieldPlaceholder) as? UILabel) != nil {
    ...
    /// Set padding for the UITextView
       func setPadding(top: CGFloat, left: CGFloat, bottom: CGFloat, right: CGFloat) {
           self.textContainerInset = UIEdgeInsets(top: top, left: left, bottom: bottom, right: right)
           self.scrollIndicatorInsets = self.textContainerInset // Adjust scroll indicators as well
       }
}
